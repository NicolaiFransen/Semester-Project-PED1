...
\todo{for the future: compare with the values the simulation with the experiment in one graph. but consider that the simulation should run for 40 seconds to plot both in one graph}
...
In the left graph in figure \ref{MPPTtestbuckmode2} is to be observed the relationship between the input and output voltage.
