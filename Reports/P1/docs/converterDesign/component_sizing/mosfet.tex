...
Now the junction temperature based on the power dissipation calculated using the SPICE model is calculated. The case \todo{maybe change case with ambient since case might be interpreted as encapsulado ¿? AT} temperature is set to 50 $\decC$, which is considered a realistic scenario. The thermal circuit can be seen in figure \ref{thermal_circuit}. The next step is to choose a commercial heat sink \todo{It is a good idea to show the temperature that the mosfets would reach without heatsink to show why it is important AT}. The constraints are thermal resistance, size and price. TDEX6015/TH was found. Its features might be found in table \ref{heatsink_features}. The switch temperature will be analysed in order to validate the heat sink.
