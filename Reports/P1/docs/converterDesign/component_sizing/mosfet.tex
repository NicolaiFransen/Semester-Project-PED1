...
The system must regulate the power flow in order to maximize the power generation \todo{power flow? Better the system must regulate input voltage and current to... AT}. In order to achieve this, the system includes switches that control the current flow \todo{switches themselves don't control the current flow, the whole topology does so. AT}. The switches consist on MOSFET devices. The switching frequency of the system is $50 $ kHz. Although the market has IGBT which can switch at $50$ kHz, MOSFET devices allow lower losses than IGBTs for system's current rating. \cite{mosfet_igbt_switching_loss} \cite{igbt_or_mosfet}
...
\end{table}
\todo{Power dissipation 150W?¿ what does that mean? AT}
...
The switching losses depend upon the switching frequency and the transistor's manufacturing characteristics. In order to calculate the value, the MOSFET's SPICE model was obtained from the manufacturer's website. The next step was to perform the simulation of the system \todo{how was this simulation performed? Stef}. The system was simulated in both Buck and Boost modes. Special attention was put into the dead-band between PWM signals of different switches, to avoid current shoot through. After simulating, the average power dissipation under steady state\todo{why steady state? Stef} was calculated in both Buck and Boost modes. Within every mode, the simulation was performed under the most unfavorable conditions, this is: Buck's output is 24 V and Boost's output is 90 V.\todo{Is it possible that low voltage at input, meaning low power may result in more unfavorable conditions? I mean, maybe going from 5V -> 90V is worst than 32V to 90V even if the power is less... AT} The results can be seen in table \ref{mosfet_power_consumption}.
...
\todo{Since you have made the simulations it would be a good idea to add them as appendices and refer to them. AT}

\begin{equation} \label{total_losses}
\overline{P} = \overline{P_{loss, T = K}} + \overline{i(t)^2 \cdot \Updelta R_{DS}}
\end{equation}

Now the junction temperature based on the power dissipation calculated using the SPICE model is calculated. The case \todo{maybe change case with ambient since case might be interpreted as encapsulado ¿? AT} temperature is set to 50 $\decC$, which is considered a realistic scenario. The thermal circuit can be seen in figure \ref{thermal_circuit}. The next step is to choose a commercial heat sink \todo{It is a good idea to show the temperature that the mosfets would reach without heatsink to show why it is important AT}. The constraints are thermal resistance, size and price. TDEX6015/TH \todo{include datasheet AT} was found. Its features might be found in table \ref{heatsink_features}. The switch temperature will be analysed in order to validate the heat sink.
...
\end{equation}
\todo{revise the upper equation, + and * are together. Also include units in the lower equation.}
...
The resistance \todo{increase on the drain to source resistance AT} is calculated as explained in equation \ref{delta_resistance}. The resistance difference is relatively small.

\begin{equation} \label{delta_resistance}
\Updelta R_{DS} = R_{DS, T = 20 \decC} - R_{DS, T = 61.41\decC} = 4\; m \Omega
\end{equation}
\todo{either absolute value or the result should be negative, consider doing Rds60º - Rds20º instead.}\todo{what are the values? you got them in simulation? Stef}

\begin{figure}[htbp]
	\begin{center}
		\includegraphics[width=0.6\textwidth]{../Pictures/thermal_circuit.png}
