...
\begin{itemize}
	...
	\item[--] Results obtained under a sudden change in the solar irradiance.
	\item[--] Results obtained under a sudden change in the cell's temperature.
\end{itemize}
...
The same simulation was implemented but in this case using a value of the resistive load of $27\Omega$. From figure \ref{boostSTC} it is observed that the MPPT needs more time than before to converge to the MPP. The P\&O algorithm needs 4 seconds to reach the MPP which means double of the time than with $R_{L}=3\Omega$. This occurs because.... ?????\todo{I don't know why it is oscillating that much in boost mode.Is it because the control variable increases every 0.01s and to reach the boost mode it has to have a value higher than 0.5? so in all the cases the boost mode will need higher time to reach the MPP because the control variable is initialize to 0. Stef} 
Despite the higher converging time to the MPP the P\&O algorithm performance has not experience a significant difference. The MPPT is able to operate with an efficiency of $\eta_{MPPT} = 99.82\% $\todo{P=299.47.Stef}
...
\end{figure}
 ...
 \subsubsection*{Change in irradiance 1000-800 $W/ m^2$}
...
\fi
 ...
 \subsubsection*{Change in temperature: 25-45 $\dec$C}
