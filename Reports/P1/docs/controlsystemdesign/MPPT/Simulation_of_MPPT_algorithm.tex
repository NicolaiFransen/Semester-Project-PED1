...
\begin{figure}[H]
	...
	\end{minipage} \\ % Captions og labels
	\begin{minipage}[t]{0.6\textwidth}
		\caption{$V_{pv}$, $I_{pv}$ and $P_{pv}$ for irradiance step 1000-800 $W/ m^2$($R_{L}=3\Omega$).} % Left caption and label
		\label{buckirradiance}
	\end{minipage}%
	\hfill
	\begin{minipage}[t]{0.6\textwidth}
		\caption{$V_{pv}$, $I_{pv}$ and $P_{pv}$ for irradiance step 1000-800 $W/ m^2$($R_{L}=27\Omega$).} % Right caption and label
