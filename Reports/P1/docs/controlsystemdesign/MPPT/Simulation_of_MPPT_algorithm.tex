...
One of the most common methods for modeling a solar panel is using the equivalent circuit of a solar cell shown in figure \ref{fig:eq_circuit_PVcell}. It is observed that the model of the solar cell can be represented as a current source connected in antiparallel with an ideal p-n junction diode \cite{MPPTResearch}. In addition, to model the non-linear behaviour of the solar panel I-V curve the equivalent series and parallel resistance ($R_{s}$ and $R_{p}$) are inserted \cite{MPPTResearch}.

\todo{wouldn't all the current flow through the diode? AT. When there is no sun or almost not light the cell behaves as a diode with a behaviour based on the Shockley equation. When there is irradiance then it's modelled by a current source proportional to to the photogenerated current. When the irradiance increases then it's seen as a superposition of both behaviours(when there is light and when there isn't resulting in the model of figure 5.4). I don't know if you understand it better like that but I dont think we should write about it in that detail. I've tried to find a reference where it explains it in more DETAIL but I've just found one in spanish. It's in bibliography papers, MPPT, spanish report PV panel model. Stef}

\begin{figure}[H]
	\begin{center}
		\includegraphics[width=0.8\textwidth]{../Pictures/schematic_solar_cell}
		\caption{Equivalent circuit of a PV cell.}
		\label{fig:eq_circuit_PVcell} 
	\end{center}	
\end{figure}

The mathematical equation that describes the previous equivalent circuit is obtained applying Kirchoff law to separate the current flowing through the different components of the circuit. This results in equation \ref{eq:pv_cell} where $I_{pv}$ is the panel's output current, $I_{ph}$ is the photogenerated current, the second term is the current flow in the diode and the last term is the current flow in the parallel resistance \cite{MPPTResearch}. 
...
The second term corresponds to the Shockley equation where $I_{o}$ is the saturation current of the diode, $a$ is the p-n junction quality factor  and $V_{t}$ is the thermal voltage defined by
...
Using the aforementioned model of the PV panel, it is possible to obtain the characteristic curves of the panel.  Usually, PV panels are tested under standard test conditions (STC) to indicate the performance of the PV modules. The STC test is carried out at a solar cell's temperature of $25^\circ$C and at a solar irradiance of 1000 $W/ m^2$ \cite{handbook}. The higher the solar irradiance more power is generated from the PV panel. On the other hand, when the temperature of the PV cell is higher than $25^\circ$C, the PV panel generates less power than at a lower temperature \cite{handbook}. 
...
\begin{figure}[H]
	\begin{center}
		\includegraphics[width=1\textwidth]{../Pictures/constant_irradiance}
		\caption{P-V curves for constant irradiance (1000$W/ m^2$) and change in temperature.}
		\label{fig:PVcurves_Irr1000} 
	\end{center}	
\end{figure}
...
\begin{figure}[H]
	\begin{center}
		\includegraphics[width=1\textwidth]{../Pictures/iv_constant_irradiance}
		\caption{I-V curves for constant irradiance (1000$W/ m^2$) and change in temperature.}
		\label{fig:IVcurves_Irr1000} 
	\end{center}	
\end{figure}
...
\begin{figure}[H]
	...
	\end{minipage} \\ % Captions og labels
	\begin{minipage}[t]{0.6\textwidth}
		\caption{Change in irradiance 1000-800$W/ m^2$ ($R_{L}=3\Omega$).} % Left caption and label
		\label{buckirradiance}
	\end{minipage}%
	\hfill
	\begin{minipage}[t]{0.6\textwidth}
		\caption{Change in irradiance 1000-800$W/ m^2$ ($R_{L}=27\Omega$).} % Right caption and label
...
\begin{figure}[H]
	...
	\end{minipage} \\ % Captions og labels
	\begin{minipage}[t]{0.6\textwidth}
		\caption{Change in temperature 25-35$\dec C$ ($R_{L}=3\Omega$).} % Left caption and label
		\label{bucktemperature}
	\end{minipage}%
	\hfill
	\begin{minipage}[t]{0.6\textwidth}
		\caption{Change in temperature 25-35$\dec C$ ($R_{L}=27\Omega$).} % Right caption and label
